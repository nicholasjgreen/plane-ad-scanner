...
- Q: Aircraft listings rarely state "IFR certified" explicitly — how should IFR approval be inferred? → A: Infer from aircraft type certification (standard-category vs permit/experimental) combined with presence of IFR-required instruments; do not rely on explicit regulatory language in listing text
...
- **FR-004**: IFR approval MUST be one of: `VFR Only`, `IFR Equipped (Not Approved)`, `IFR Approved`, `Unknown` — reflecting the regulatory and equipment status of the aircraft. Listings rarely state IFR approval explicitly; the AI MUST infer it from aircraft type certification and equipment, using this decision framework:
  - `IFR Approved`: Standard-category (type-certificated) aircraft AND listing describes the minimum IFR instrument set — attitude indicator (or AHRS), directional gyro (or HSI), sensitive altimeter, turn coordinator, and at least one IFR nav source (VOR/ILS receiver, or WAAS GPS with approach capability). The combination of type certificate + IFR instruments = approved, even if the seller does not say so.
  - `IFR Equipped (Not Approved)`: Aircraft is Permit to Fly or Experimental category (see FR-023) AND has IFR-capable instruments; OR is a standard-category aircraft but the equipment list clearly falls short of the minimum IFR set (e.g. has a WAAS GPS but no attitude indicator or turn coordinator mentioned).
  - `VFR Only`: Equipment list describes only VFR instruments (altimeter, ASI, VSI, compass, no attitude indicator or IFR nav source); or aircraft is a microlight/ultralight/SSEA category; or listing explicitly states VFR only.
  - `Unknown`: Equipment list is too sparse to determine whether the minimum IFR instrument set is present.
...
- IFR approval (FR-004) is almost never stated explicitly in listing text. The AI derives it by combining airworthiness basis (FR-023) with the instrument/avionics equipment described. Explicit phrases like "IFR certified" are supporting evidence when present but must not be required.
