...
**Independent Test**: Can be tested by seeding the database with a set of listings and confirming each renders a card with a non-generic headline, four key fields (make, model, year, price), and at least one thumbnail; and that none of these are blank or show raw field names.

**Acceptance Scenarios**:

1. **Given** a listing has been scanned and stored, **When** I view the listing page, **Then** each listing appears as a card showing a generated headline, make, model, year, price, and one or more thumbnail images.
...
1. **Given** I update an interest profile, **When** I next view the expanded explanation for a stored listing, **Then** the explanation reflects the updated profile rather than the version active when the listing was first found.
2. **Given** explanations are regenerated after a profile update, **When** regeneration is in progress, **Then** the page still loads and shows the previous explanation until the new one is ready.

---

### Edge Cases

- What should the headline say when the listing has very little data (e.g. only a price and a URL)?
...
- **FR-002**: The headline MUST reflect the listing's notable attributes (aircraft role, ownership type, location, etc.) and MUST NOT be a generic or repeated placeholder.
...
- **FR-010**: Explanations MUST be regenerated when the user's active interest profiles change, so the reasoning always reflects current interests.
...
- **ListingCard**: The summary presentation of a listing — generated headline, make, model, year, asking price, thumbnail image URL(s), and link to expanded view.
...
- **SC-004**: After an interest profile update, all listing explanations reflect the updated profile within one scan cycle or on the next page load.
